...
trait InitUtils {
  ...
  def printParams[F[_]: Sync: Logger](
    ...
    for {
      ...
      _ <- info"plasma-blocks-to-recover   : ${params.toplWaitExpirationTime}"
      _ <-
        info"btc-confirmation-threshold  : ${params.btcConfirmationThreshold}"
      _ <-
        info"plasma-confirmation-threshold : ${params.toplConfirmationThreshold}"
      ...
      _ <- info"plasma-wallet-seed-file    : ${params.toplWalletSeedFile}"
      _ <- info"plasma-wallet-password     : ******"
      _ <- info"plasma-wallet-db           : ${params.toplWalletDb}"
      ...
      _ <- info"plasma-network             : ${params.toplNetwork}"
      _ <- info"plasma-host                : ${params.toplHost}"
      _ <- info"plasma-port                : ${params.toplPort}"
      _ <- info"config-file              : ${params.configurationFile.toPath().toString()}"
      _ <- info"plasma-secure-connection   : ${params.toplSecureConnection}"
