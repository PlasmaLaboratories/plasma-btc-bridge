...
package object utils {
  ...
  def getConfig(container: String): IO[String] = runProcess(
    Seq("docker", "exec", container, "cat", "/node-staking/config.yaml")
  )

  def updateConfig(container: String, newConfig: String): IO[Unit] = runProcess(
    Seq("docker", "exec", container, "sh", "-c", "\"echo", newConfig, ">", "/node-staking/config.yaml\"")
  ).void
  ...
  def connectNodeNodes(node: String, otherNode: String): IO[Unit] = for {
  ...
  def disconnectNodeNodes(node: String): IO[Unit] = for {
    oldConfig <- getConfig(node)
