...
echo $(pwd)
export CONTAINER_ID=$(docker run -d --name node01 -p 9185:9085 -p 9184:9084 -p 9191:9091 -v $(pwd)/node01:/node-staking:rw docker.io/stratalab/plasma-node:0.1.3 --  --config=/node-staking/config.yaml --regtest)
...
docker run -d --name node02 -p 9087:9085 -p 9086:9084 -p 9092:9091 -v $(pwd)/node02:/node-staking docker.io/stratalab/plasma-node:0.1.3 --  --config  /node-staking/config.yaml --regtest
