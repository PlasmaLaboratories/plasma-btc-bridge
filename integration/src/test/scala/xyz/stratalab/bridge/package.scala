...
package object bridge extends ProcessOps {
  ...
  val csParams = Seq(
    ...
    "org.plasmalabs:sdk-cli_2.13:2.0.0-beta6",
    "--"
  )
  ...
  // sdk-cli wallet init --network private --password password --newwalletdb user-wallet.db --mnemonicfile user-wallet-mnemonic.txt --output user-wallet.json
  ...
  // sdk-cli templates add --walletdb user-wallet.db --template-name redeemBridge --lock-template
  ...
  // sdk-cli wallet import-vks --walletdb user-wallet.db --input-vks key.txt --fellowship-name bridge --template-name redeemBridge -w password -k user-wallet.json
  ...
  // sdk-cli wallet current-address --walletdb user-wallet.db
  ...
  // sdk-cli simple-transaction create --from-fellowship bridge --from-template redeemBridge -t ptetP7jshHTzLLp81RbPkeHKWFJWeE3ijH94TAmiBRPTUTj2htC31NyEWU8p -w password -o redeemTx.pbuf -n private -a 10 -h  localhost --port 9084  --keyfile user-keyfile.json --walletdb user-wallet.db --fee 10 --transfer-token asset
  ...
  // sdk-cli tx prove -i fundRedeemTx.pbuf --walletdb user-wallet.db --keyfile user-keyfile.json -w password -o fundRedeemTxProved.pbuf
  ...
  // sdk-cli tx broadcast -i fundRedeemTxProved.pbuf -h localhost --port 9084
