...
export CONTAINER_ID=`docker run --rm -d --name node01 -p 9085:9085 -p 9084:9084 -p 9091:9091 -v $(pwd)/node01:/staking:rw stratalab/strata-node-tooling:0.0.0-8200-7a9041ed --  --config  /staking/config.yaml --regtest`
...
docker run --rm -d --name node02 -e BIFROST_P2P_KNOWN_PEERS=$IP_CONTAINER:9085 -p 9087:9085 -p 9086:9084 -p 9092:9091 -v $(pwd)/node02:/staking:rw stratalab/strata-node-tooling:0.0.0-8200-7a9041ed --  --config  /staking/config.yaml --regtest
...
alias sdk-cli="cs launch -r https://s01.oss.sonatype.org/content/repositories/releases org.plasmalabs:sdk-cli_2.13:2.0.0-beta5+2-4f603d9d-SNAPSHOT -- "
...
sdk-cli node-query mint-block --nb-blocks 1 -h 127.0.0.1  --port 9084 -s false
...
sdk-cli wallet init --network private --password password --newwalletdb $STRATA_WALLET_DB --mnemonicfile $STRATA_WALLET_MNEMONIC --output $STRATA_WALLET_JSON
export ADDRESS=$(sdk-cli wallet current-address --walletdb $STRATA_WALLET_DB)
sdk-cli simple-transaction create --from-fellowship nofellowship --from-template genesis --from-interaction 1 --change-fellowship nofellowship --change-template genesis --change-interaction 1  -t $ADDRESS -w $STRATA_WALLET_PASSWORD -o genesisTx.pbuf -n private -a 1000 -h  127.0.0.1 --port 9084 --keyfile $STRATA_WALLET_JSON --walletdb $STRATA_WALLET_DB --fee 10 --transfer-token lvl
sdk-cli tx prove -i genesisTx.pbuf --walletdb $STRATA_WALLET_DB --keyfile $STRATA_WALLET_JSON -w $STRATA_WALLET_PASSWORD -o genesisTxProved.pbuf
export GROUP_UTXO=$(sdk-cli tx broadcast -i genesisTxProved.pbuf -h 127.0.0.1 --port 9084)
sdk-cli node-query mint-block --nb-blocks 1 -h 127.0.0.1  --port 9084 -s false
until sdk-cli indexer-query utxo-by-address --host localhost --port 9084 --secure false --walletdb $STRATA_WALLET_DB; do sleep 5; done
echo "label: ToplBTCGroup" > groupPolicy.yaml
echo "registrationUtxo: $GROUP_UTXO#0" >> groupPolicy.yaml
sdk-cli simple-minting create --from-fellowship self --from-template default  -h 127.0.0.1 --port 9084 -n private --keyfile $STRATA_WALLET_JSON -w $STRATA_WALLET_PASSWORD -o groupMintingtx.pbuf -i groupPolicy.yaml  --mint-amount 1 --fee 10 --walletdb $STRATA_WALLET_DB --mint-token group
sdk-cli tx prove -i groupMintingtx.pbuf --walletdb $STRATA_WALLET_DB --keyfile $STRATA_WALLET_JSON -w $STRATA_WALLET_PASSWORD -o groupMintingtxProved.pbuf
export SERIES_UTXO=$(sdk-cli tx broadcast -i groupMintingtxProved.pbuf -h 127.0.0.1 --port 9084)
sdk-cli node-query mint-block --nb-blocks 1 -h 127.0.0.1  --port 9084 -s false
...
sdk-cli simple-minting create --from-fellowship self --from-template default  -h localhost --port 9084 -n private --keyfile $STRATA_WALLET_JSON -w $STRATA_WALLET_PASSWORD -o seriesMintingTx.pbuf -i seriesPolicy.yaml  --mint-amount 1 --fee 10 --walletdb $STRATA_WALLET_DB --mint-token series
sdk-cli tx prove -i seriesMintingTx.pbuf --walletdb $STRATA_WALLET_DB --keyfile $STRATA_WALLET_JSON -w $STRATA_WALLET_PASSWORD -o seriesMintingTxProved.pbuf
export ASSET_UTXO=$(sdk-cli tx broadcast -i seriesMintingTxProved.pbuf -h 127.0.0.1 --port 9084)
sdk-cli node-query mint-block --nb-blocks 1 -h 127.0.0.1  --port 9084 -s false
